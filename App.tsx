...
const App = () => {
  ...
  const [savedStores, setSavedStores] = useState<string[]>(() => {
    ...
    return saved ? JSON.parse(saved) : ['Mercado A', 'Atacado B']; // Initial defaults
  });
  ...
  useEffect(() => {
    if (activeTab === 'activity') {
        ...
        if (hasUnread) {
            // Small delay to let user see the badge disappear naturally
            setTimeout(() => {
  ...
  const handleSaveItem = (
    ...
    if (editId) {
        ...
        if (existingItem) {
          ...
          setInventory(prev => prev.map(item => 
            item.id === itemId 
              ? { ...item, quantity: item.quantity + itemData.quantity, lastUpdated: new Date().toISOString() }
              : item
          ));
      ...
      {shoppingList.length === 0 ? (
        ...
        shoppingList.map(item => (
          <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border-l-4 border-orange-400 flex justify-between items-center">
             ...
             <button 
                onClick={() => {
                   setEditingItem(null); // Ensure we are not editing
                   // Pre-fill modal would be better, but just opening is fine for now
                   setIsModalOpen(true);
                }}
